...
def main():
    ...
    # 1. Mandatory: each prompt × each model
    for prompt_id, prompt in prompts:
        for model_name in model_names:
            jobs.append((prompt_id, prompt, model_name))

    # 2. Random noisy traffic: same count as prompts (or increase if needed)
    for prompt_id, prompt in prompts:
        rand_model = random.choice(model_names)
        jobs.append((prompt_id, prompt, rand_model))

    # 3. Shuffle everything so mandatory + random fully intermingle
    random.shuffle(jobs)

    for prompt_id, prompt, model_name in jobs:
