...
from .metrics_watcher import start_metrics_watcher, model_metrics, track_sent_request
from .load_patterns import RequestPattern
from .cost_model import HardwareCostPredictor
from .model_maps import get_model_id, get_model_hugging_face_name

# =========================================================
# Normalization constants (from h100_full_sweep)
# =========================================================
...
# =========================================================
# Model price per output token (same as training)
# =========================================================
MODEL_PRICES = {
...
def run_eval(
    ...
    def worker(prompt_source_id, prompt):
        ...
        for local_model_name in model_names:
            ...
            # Use token-weighted waiting queue estimate instead of count
            waiting_tokens_estimate = snap.get("waiting_tokens_estimate", 0.0)
            # Normalize by current request size to get "equivalent requests"
            waiting = waiting_tokens_estimate / max(p_tokens, 1.0)
        ...
        # Track this request for the chosen model (for sliding window token estimation)
        track_sent_request(best_choice["local_model"], p_tokens)
